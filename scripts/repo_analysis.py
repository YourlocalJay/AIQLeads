        ...
        for filepath in self.repo_path.rglob('*.py'):
            if not any(p.startswith('.') for p in filepath.parts):
                try:
                    ...
                    module_name = str(filepath.relative_to(self.repo_path)).replace('/', '.').replace('.py', '')
...
if __name__ == "__main__":
    ...
    print(f"Duplicate Files: {sum(len(files) for files in report['duplicate_files'].values())}")
