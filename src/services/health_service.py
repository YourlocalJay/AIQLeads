...
class HealthService:
    ...
    def _on_retry(self, error: Exception, attempt: int) -> None:
        ...
        logger.warning(
            f"Retry attempt {attempt} failed with error: {str(error)}"
        )
    ...
    async def check_all_services(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            'cache': results[1] if not isinstance(results[1], Exception) else {'status': 'unhealthy', 'error': str(results[1])},
            'search': results[2] if not isinstance(results[2], Exception) else {'status': 'unhealthy', 'error': str(results[2])},
        ...
        status['circuit_breakers'] = {
            'database': {'state': self.db_circuit.state, 'failures': self.db_circuit.failures},
            'cache': {'state': self.cache_circuit.state, 'failures': self.cache_circuit.failures},
            'search': {'state': self.search_circuit.state, 'failures': self.search_circuit.failures}
        }
