...
class TransactionCreate(BaseModel):
    ...
    currency: str = Field(
        ..., description="Three-letter ISO currency code", min_length=3, max_length=3
    )
    type: TransactionType = Field(..., description="Type of transaction")
    reference_id: str = Field(
        ...,
        description="Unique reference ID from payment processor",
        max_length=100,
        regex="^[A-Za-z0-9_-]+$",
    )
    description: Optional[str] = Field(
        None, description="Transaction description", max_length=255
    )
