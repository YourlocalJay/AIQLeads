    ...
    async def extract_captcha_image(response) -> Optional[str]:
        try:
            if isinstance(response, str):
                ...
                html_content = response.text if hasattr(response, 'text') else await response.content()
...
class PerformanceMetricsAggregator:
    def __init__(self):
        ...
        self.proxy_performance = prometheus_metrics.Gauge(
            'proxy_performance_score', 'Performance score for each proxy per domain', ['domain', 'proxy']
        )
        self.scrape_success_rate = prometheus_metrics.Histogram(
            'scrape_success_rate', 'Success rate of scraping attempts', ['domain']
        )

    def record_domain_error(self, domain: str, error_type: str, details: Optional[str] = None):
...
class PaginationHandler:
    ...
    async def handle_pagination(
        scraper, base_url: str, max_pages: int = 10, pagination_selector: Optional[str] = None
        ...
        while current_page <= max_pages:
            try:
                ...
                if not page_data:
                    break
                yield page_data
                next_url = await scraper._find_next_page_url(current_url, page_data, pagination_selector)
                if not next_url:
                    break
...
class BaseScraper(ABC):
    ...
    async def _find_next_page_url(
        self, current_url: str, page_data: Dict[str, Any], pagination_selector: Optional[str] = None
    ) -> Optional[str]:
    ...
    async def scrape_paginated(
        self, base_url: str, max_pages: int = 10, pagination_selector: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        all_data = []
