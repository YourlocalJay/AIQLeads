...
class AggregationPipeline:
    ...
    async def initialize_components(self) -> None:
        ...
        try:
            ...
            scraper_configs = {
                ...
                "fsbo": FSBOScraper(base_url=self.settings.FSBO_BASE_URL),
            }
    ...
    async def _process_source(
        self, source: str, location: str, radius_km: float, **kwargs
    ) -> List[LeadCreate]:
        """
        ...
        try:
            ...
            raw_leads = await scraper.search(
                location=location, radius_km=radius_km, **kwargs
            )
    ...
    async def _parse_lead(
        self, raw_lead: Dict[str, Any], parser: BaseParser
    ) -> LeadCreate:
        """
    ...
    async def get_recommendations(
        self, user_id: str, leads: List[Dict[str, Any]], limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            return await self.recommendation_engine.get_recommendations(
                user_id=user_id, available_leads=leads, limit=limit
            )
        except Exception as e:
    ...
    async def track_user_behavior(
        ...
        await self.recommendation_engine.track_user_behavior(
            user_id=user_id, action=action, lead_id=lead_id, metadata=metadata
        )
