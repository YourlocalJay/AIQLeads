...
class ZillowScraper(BaseScraper):
    ...
    async def search(
        ...
        try:
            logger.info(
                f"Starting Zillow search for location: {location}, radius: {radius_km} km"
            )
    ...
    def _build_filters(self, **kwargs) -> Dict[str, Any]:
        ...
        filters = {
            ...
            "price": {"min": kwargs.get("price_min"), "max": kwargs.get("price_max")},
        }
        # Remove None values
    ...
    async def _parse_listings(self, data: Dict[str, Any]) -> List[LeadCreate]:
        ...
        for listing in listings:
            try:
                ...
                self.add_error(
                    "parse_error",
                    f"Failed to parse listing: {str(e)}",
