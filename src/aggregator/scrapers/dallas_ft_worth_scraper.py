...
class DallasFtWorthScraper(BaseScraper):
    """Scraper for extracting real estate leads from Dallas/Ft. Worth and surrounding areas."""
    
    BASE_URL = "https://www.dallasrealestate.com/api/v1/properties"
    SUBREGIONS = ["Plano", "Frisco", "Arlington", "Fort Worth"]

    def __init__(self, rate_limit: int = 50, time_window: int = 60):
        super().__init__(rate_limit, time_window)
    
    async def search(self, location: str = "Dallas", radius_km: float = 50.0, include_subregions: bool = True, **kwargs) -> List[LeadCreate]:
        """
        Search for properties in Dallas/Ft. Worth and optional surrounding areas.

        Args:
            location (str): Central target location (default: "Dallas").
        ...
        """
        try:
            logger.info(f"Starting Dallas/Ft. Worth search for location: {location}, radius: {radius_km} km")
            ...
            logger.error(f"Dallas/Ft. Worth scraper failed: {e}")
            raise ScraperError("Dallas/Ft. Worth scraper failed.")
    ...
    async def validate_credentials(self) -> bool:
        ...
        Returns:
            bool: Always True for Dallas/Ft. Worth scraper.
        """
        logger.info("Validating credentials for Dallas/Ft. Worth scraper.")
    ...
    async def extract_contact_info(self, listing_data: dict) -> dict:
        ...
        Args:
            listing_data (dict): Raw data from Dallas/Ft. Worth listings.
    ...
    async def _mock_api_call(self, location: str, radius_km: float) -> List[dict]:
        ...
        return [
            {"agentName": "Charlie Adams", "agentEmail": "charlie.adams@example.com", "agentPhone": "+5555555555", "price": 450000},
            {"agentName": "Dana Lee", "agentEmail": "dana.lee@example.com", "agentPhone": "+6666666666", "price": 550000},
        ]
    ...
    def _parse_listing(self, listing: dict) -> LeadCreate:
        ...
        try:
            return LeadCreate(
                ...
                source="DallasRealEstate",
                metadata={"price": listing.get("price"), "city": "Dallas/Ft. Worth"}
            )
