...
class ProxyManager:
    ...
    def __init__(self, proxies: Optional[List[str]] = None):
        ...
        self._proxy_performance: Dict[str, Dict[str, float]] = defaultdict(lambda: defaultdict(float))
        self._last_used_time: Dict[str, Dict[str, datetime]] = defaultdict(lambda: defaultdict(datetime.now))
        self._failure_counts: Dict[str, Dict[str, int]] = defaultdict(lambda: defaultdict(int))
        self._banned_until: Dict[str, Dict[str, datetime]] = defaultdict(lambda: defaultdict(datetime.now))
    ...
    def get_best_proxy(self, domain: str) -> Optional[str]:
        ...
        if not available_proxies:
            all_banned = all(current_time < self._banned_until[domain][p] for p in self._proxies)
            if all_banned:
    ...
    def report_proxy_success(self, domain: str, proxy: str):
        ...
        self._failure_counts[domain][proxy] = max(0, self._failure_counts[domain][proxy] - 1)

    def report_proxy_failure(self, domain: str, proxy: str, status_code: Optional[int] = None):
        """
        ...
        if failures >= 3:
            ban_duration = min(30 * (2 ** (failures - 3)), 3600)  # Exponential backoff, max 1 hour
            self._banned_until[domain][proxy] = datetime.now() + timedelta(seconds=ban_duration)
    ...
    def get_proxy_stats(self, domain: str, proxy: str) -> Dict[str, float]:
        ...
        return {
            ...
            "banned_remaining": max(
                0, 
                (self._banned_until[domain][proxy] - current_time).total_seconds()
            )
        }