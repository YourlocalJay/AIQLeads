...
class ExpansionForecaster:
    ...
    async def _analyze_competitor_presence(self, region_id: str) -> Dict:
        ...
        try:
            ...
            total_market_share = sum(c.market_share for c in competitor_activities.values())
            avg_growth_rate = np.mean([c.growth_rate for c in competitor_activities.values()])
            ...
            for opportunity in opportunities:
                if opportunity['type'] == 'market_gap':
                    recommendations.append({
                        'type': 'expansion',
                        'priority': 'high' if opportunity['score'] > 0.7 else 'medium',
                        'action': 'Increase market presence',
                        'details': {
                            'target_market_share': min(0.3, opportunity['score']),
                            'timeframe': '3 months' if opportunity['score'] > 0.7 else '6 months',
                            'resource_allocation': self._calculate_resource_allocation(opportunity['score'])
                        }
                    })
                    ...
                    recommendations.append({
                        ...
                        'details': {
                            ...
                            'focus_areas': self._identify_growth_areas(expansion_score)
                        }
                    })
    ...
    async def _calculate_growth_trajectory(self, region_id: str) -> float:
        ...
        try:
            ...
            for i in range(1, len(history)):
                growth_rate = (
                    (history[i].lead_density - history[i-1].lead_density) / history[i-1].lead_density +
                    (history[i].engagement_rate - history[i-1].engagement_rate) / history[i-1].engagement_rate +
                    (history[i].conversion_rate - history[i-1].conversion_rate) / history[i-1].conversion_rate
                ) / 3.0
    ...
    def _calculate_expansion_threat(self, competitor_activities: Dict[str, CompetitorActivity]) -> float:
