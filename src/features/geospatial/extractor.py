...
class GeospatialFeatureExtractor(BaseFeatureExtractor):
    ...
    async def _core_features(self, context: GeospatialContext) -> Dict[str, float]:
        ...
        return {
            ...
            "area_density": (
                count / (3.14159 * (context.radius_km**2))
                if context.radius_km > 0
                else 0
            ),
        }
    ...
    async def _market_distance(self, market_id: int, point: Geography) -> float:
        ...
        return (
            await self.session.scalar(select(func.ST_Distance(point, market_center)))
    ...
    async def _nearby_density(self, point: Geography, radius_meters: float) -> int:
        ...
        return await self.session.scalar(
            select(func.count()).where(
                ST_DWithin(Lead.geolocation.cast(Geography), point, radius_meters)
            )
        )

    async def _calculate_coverage(
        self, context: GeospatialContext, boundary: Geography
    ) -> float:
        ...
        buffer = ST_Buffer(
            self._create_geo_point(context).cast(Geography), context.radius_km * 1000
        )
        return (
    ...
    def _create_geo_point(self, context: GeospatialContext) -> Geography:
        ...
        return ST_SetSRID(
            ST_MakePoint(context.location[1], context.location[0]), 4326
        ).cast(Geography)
