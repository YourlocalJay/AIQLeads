...
class SchemaMetrics:
    def __init__(self):
        ...
        self.validation_duration = Histogram(
            ...
            buckets=(
                ...
                0.05,
                0.1,
            ),  # Fine-grained buckets for <10ms target
        )
    ...
    def track_validation_time(self, endpoint: str, schema_version: str):
        ...
        try:
            ...
            if duration > 0.01:  # 10ms threshold
                logger.warning(
                    f"Schema validation exceeded 10ms threshold: {duration:.4f}s for {endpoint}"
                )
    ...
    def track_cache_operation(self, operation: str, hit: bool):
        ...
        if operation == "get":
            current_hits = self.cache_operations.labels(
                operation="get", status="hit"
            )._value.get()
            total_gets = (
                self.cache_operations.labels(operation="get", status="hit")._value.get()
    ...
    def record_validation_error(
        self, endpoint: str, error_type: str, schema_version: str
    ):
        self.validation_errors.labels(
            endpoint=endpoint, error_type=error_type, schema_version=schema_version
        ).inc()

        logger.error(
            f"Schema validation error: {error_type} for {endpoint} (version: {schema_version})"
        )
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                metrics.record_validation_error(
                    endpoint, type(e).__name__, schema_version
                )
                raise
