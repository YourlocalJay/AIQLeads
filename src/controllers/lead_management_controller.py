...
router = APIRouter(
    prefix="/leads", tags=["Leads"], dependencies=[Depends(validate_api_key)]
)
...
@router.post(
    "/batch", response_model=List[LeadResponse], summary="Create multiple leads"
)
...
async def create_leads_batch(
    ...
    try:
        ...
        background_tasks.add_task(
            track_lead_source_performance, [l.lead_source for l in valid_leads]
        )
...
async def get_priority_queue(
    ...
    try:
        ...
        result = await session.execute(
            sa.select(Lead)
            .where(sa.and_(Lead.score >= min_score, Lead.created_at >= min_date))
