...
async def get_market_forecast(
    ...
    try:
        ...
        forecast = await predict_market_growth(
            historical_data=historical_data, forecast_days=forecast_days
        )

        # Cache results
        await redis_client.set(cache_key, forecast.json(), ex=CACHE_TTL)
...
async def compare_regions(
    ...
    try:
        ...
        # Cache results
        await redis_client.set(cache_key, json.dumps(comparison), ex=CACHE_TTL)
...
@router.get("/{region_name}/export", summary="Export market insights")
...
def calculate_growth_rate(data: List[MarketInsight]) -> float:
    ...
    if len(data) < 2:
        return 0.0
    return (data[0].penetration_rate - data[-1].penetration_rate) / data[
        -1
    ].penetration_rate
...
def calculate_volatility(data: List[MarketInsight]) -> float:
    ...
    if len(data) < 2:
        return 0.0
    changes = [
        abs(data[i].penetration_rate - data[i - 1].penetration_rate)
