...
def _find_request(args: tuple, kwargs: dict, param_name: str) -> Optional[Request]:
    ...
    return (
        ...
        if isinstance(kwargs.get(param_name), Request)
        else next((arg for arg in args if isinstance(arg, Request)), None)
    )
...
def _recursive_redact(data: Any, redact: set) -> Any:
    ...
    if isinstance(data, dict):
        return {
            k: (
                _recursive_redact(v, redact)
                if k not in redact
                else f"<REDACTED {type(v).__name__}>"
            )
...
def _store_timing(request: Optional[Request], func_name: str, timing: float) -> None:
    ...
    if request:
        if not hasattr(request.state, "timings") or not isinstance(
            request.state.timings, dict
        ):
...
def _log_execution(
    func: Callable, total_time: float, args: tuple, kwargs: dict, threshold: float
) -> None:
        ...
        async def wrapper(*args, **kwargs) -> Any:
            ...
            try:
                ...
                if failures >= failure_threshold:
                    ...
                    logger.warning(
                        f"Circuit breaker activated for {func.__name__}: {failures} consecutive failures"
                    )
