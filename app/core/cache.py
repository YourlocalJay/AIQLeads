...
CACHE_OPERATION_TIME = Histogram(
    "cache_operation_seconds", "Time spent on cache operations", ["operation"]
)
CACHE_ERROR_COUNTER = Counter(
    "cache_errors_total", "Number of cache operation errors", ["operation"]
)
        ...
        if self._redis is None:
            try:
                ...
                self._redis = aioredis.Redis(
                    connection_pool=self._connection_pool, retry_on_timeout=True
                )
