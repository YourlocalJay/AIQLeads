    ...
    def _update_aggregates(self, metric_name: str, value: float, context_json: str):
        ...
        self._conn.execute("""
            ...
            SET avg_value = ((avg_value * count + ?) / (count + 1)),
                ...
                count = count + 1
        """, (period_start, metric_name, value, value, value, context_json, value, value, value))
    ...
    def _update_thresholds(self, metric_name: str):
        ...
        self._conn.execute("""
            ...
            SET lower_bound = ?,
                upper_bound = ?,
                last_updated = ?
        """, (metric_name, lower_bound, upper_bound, datetime.now(),
              lower_bound, upper_bound, datetime.now()))

    def _record_anomaly(self, metric_name: str, value: float, lower_bound: float, upper_bound: float):
        # Here you would implement anomaly recording and alerting
        # For now, we'll just print the anomaly
        print(f"ANOMALY DETECTED: {metric_name} = {value} (bounds: {lower_bound}, {upper_bound})")
