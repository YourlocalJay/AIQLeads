...
class LeadBehaviorExtractor(BaseFeatureExtractor):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.engagement_weights = config.get(
            "engagement_weights",
            {
                "view": 1.0,
                "click": 2.0,
    ...
    async def _compute_features(self, lead_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        try:
            ...
            logger.error(
                f"Feature extraction failed for lead behavior: {str(e)}", exc_info=True
            )
            raise
    ...
    def _validate_lead_data(self, lead_data: Dict[str, Any]) -> None:
        ...
        if len(lead_data["interactions"]) < self.min_interactions:
            raise ValidationError(
                f"Must have at least {self.min_interactions} interactions"
            )

    def _compute_action_frequencies(
    ...
    def _compute_engagement_metrics(
        ...
        scores = [self.engagement_weights.get(i["action"], 0.0) for i in interactions]
    ...
    def _compute_session_metrics(
        ...
        sorted_interactions = sorted(
            interactions, key=lambda x: datetime.fromisoformat(x["timestamp"])
        )
        ...
        for curr, next_int in zip(sorted_interactions, sorted_interactions[1:]):
            ...
            if (
                next_time - curr_time
            ).total_seconds() / 3600 > self.max_interaction_gap:
