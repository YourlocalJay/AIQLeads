...
class PreprocessorConfig(BaseModel):
    ...
    input_fields: List[str] = Field(
        ..., description="Fields required for preprocessing"
    )
    output_fields: List[str] = Field(
        ..., description="Fields produced by preprocessing"
    )
    validation_rules: Optional[Dict[str, Any]] = Field(
        default=None, description="Optional validation rules for input data"
    )
...
class BasePreprocessor(ABC):
    ...
    def _validate_input(self, data: Dict[str, Any]) -> None:
        ...
        """
        missing_fields = [
            field for field in self.config.input_fields if field not in data
        ]
    ...
    async def process(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            missing_outputs = [
                field for field in self.config.output_fields if field not in result
            ]
            if missing_outputs:
                raise PreprocessingError(
