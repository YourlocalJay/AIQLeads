...
class TestAIOptimizationsIntegration(unittest.TestCase):
    def setUp(self):
        ...
        self.circuit_breaker = AICircuitBreaker(failure_threshold=3, recovery_timeout=5)
    ...
    def test_cache_with_monitoring(self):
        ...
        metrics = self.monitor.get_metrics(
            "cache_operation_time", datetime.now() - timedelta(minutes=5)
        )
    ...
    def test_circuit_breaker_with_monitoring(self):
        ...
        def monitored_operation():
            ...
            try:
                ...
                self.monitor.record_metric(
                    "operation_failure", 1.0, {"error_type": type(e).__name__}
                )
                raise
            finally:
                # Record operation time
                operation_time = time.time() - start_time
                self.monitor.record_metric("operation_time", operation_time)
        ...
        failure_metrics = self.monitor.get_metrics(
            "operation_failure", datetime.now() - timedelta(minutes=5)
        )
    ...
    def test_full_system_integration(self):
        ...
        def complex_operation(key, value):
            ...
            try:
                ...
                # Record success
                self.monitor.record_metric(
                    "operation_success", 1.0, {"operation": "cache_access"}
                )
        ...
        success_metrics = self.monitor.get_metrics(
            "operation_success", datetime.now() - timedelta(minutes=5)
        )
        self.assertEqual(len(success_metrics), 20)

        # Check system performance
        perf_stats = self.monitor.get_aggregates(
            "operation_time", datetime.now() - timedelta(minutes=5)
        )
