...
import os
import sys
from github import Repository, GitRef, Commit, GitTree, GitCommit
...
def mock_github():
    ...
    with patch("github.Github") as mock:
        ...
        mock_repo.get_commits.return_value = [MagicMock(spec=Commit.Commit)]
        ...
        mock_repo.create_git_commit.return_value = MagicMock(
            spec=GitCommit.GitCommit, sha="test_commit_sha"
        )
...
def test_update_workflow_files(mock_github):
    ...
    with patch.dict(
        "os.environ", {"GITHUB_TOKEN": "test_token", "GITHUB_REPOSITORY": "test/repo"}
    ):
