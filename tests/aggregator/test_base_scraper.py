...
from src.aggregator.exceptions import NetworkError
...
async def test_error_handling(scraper):
    ...
    with patch(
        "httpx.AsyncClient.request", AsyncMock(return_value=rate_limit_response)
    ):
        result = await scraper._safe_fetch("https://test.com/api")
...
async def test_browser_fallback(scraper):
    ...
    with patch(
        ...
        with patch(
            "src.aggregator.components.browser_manager.PersistentBrowserManager.get_instance",
