...
class TestAggregatorPerformance:
    ...
    def large_dataset(self):
        ...
        return [
            self._generate_mock_listing(i) for i in range(1000)
        ]
    ...
    async def test_parser_performance(self, large_dataset, performance_metrics):
        ...
        assert all(result['leads_processed'] == size for size, result in results.items())
        assert statistics.mean(performance_metrics['throughput']) > 100  # Min 100 leads/second
    ...
    async def test_scraper_performance(self, performance_metrics):
        ...
        assert statistics.mean(performance_metrics['response_times']) < 0.1  # Max 100ms per request
    ...
    async def test_pipeline_performance(self, large_dataset, performance_metrics):
        ...
        assert all(result['leads_processed'] == size for size, result in results.items())
        assert statistics.mean(performance_metrics['throughput']) > 50  # Min 50 leads/second
        assert max(results.values(), key=lambda x: x['duration'])['duration'] < 30  # Max 30 seconds
    ...
    async def test_concurrent_source_performance(self, performance_metrics):
        ...
        facebook_scraper = FacebookScraper(api_key=settings.FACEBOOK_API_KEY, rate_limit=100)
        zillow_parser = ZillowParser()
        facebook_parser = FacebookParser()

        sources = [
            (zillow_scraper, zillow_parser),
            (facebook_scraper, facebook_parser)
        ]
        ...
        for scraper, parser in sources:
            with patch.object(scraper, 'fetch_listings') as mock_fetch:
                mock_fetch.return_value = [self._generate_mock_listing(i) for i in range(50)]
