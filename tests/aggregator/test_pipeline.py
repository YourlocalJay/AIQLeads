...
async def test_aggregate_leads_with_batch_processing(pipeline):
    ...
    for i in range(5):
        await pipeline.register_scraper(f"test{i}", MockScraper(base_url=f"https://test{i}.com"))
...
async def test_validation_handling(pipeline):
    ...
    # Register components
    await pipeline.register_scraper("invalid", InvalidLeadScraper(base_url="https://test.com"))
