...
def test_different_exceptions():
    ...
    @retry_with_backoff(
        max_retries=2, base_delay=0.1, exceptions=(ValueError, RuntimeError)
    )
    def mixed_errors():
...
async def test_concurrent_retries():
    ...
    tasks = [asyncio.create_task(concurrent_operation(i)) for i in range(3)]
...
async def test_retry_cleanup():
    ...
    @async_retry_with_backoff(max_retries=1, base_delay=0.1, cleanup_func=cleanup)
