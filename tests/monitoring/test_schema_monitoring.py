...
from .schema_monitoring import SchemaMetrics, monitor_schema_validation
...
class TestSchemaMonitoring:
    ...
    async def test_validation_time_tracking(self, schema_metrics):
        ...
        metric = schema_metrics.validation_duration.labels(endpoint=endpoint, schema_version=schema_version)
    ...
    def test_cache_hit_ratio_tracking(self, schema_metrics):
        ...
        hits = schema_metrics.cache_operations.labels(operation='get', status='hit')._value.get()
        total = hits + schema_metrics.cache_operations.labels(operation='get', status='miss')._value.get()
