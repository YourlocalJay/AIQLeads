...
def mock_system_metrics():
    ...
    ), patch("psutil.disk_io_counters", return_value=(1000, 2000)), patch(
        "psutil.net_io_counters", return_value=(3000, 4000)
    ):
        yield
...
def mock_high_load():
    with patch("psutil.cpu_percent", return_value=95.0), patch(
        "psutil.virtual_memory", return_value=Mock(percent=90.0)
    ):
        yield
