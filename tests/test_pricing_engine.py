...
def sample_lead_data():
    return {
        ...
        "verification_status": {"email": 1.0, "phone": 0.8, "address": 0.9},
        "enrichment_data": {
            "company_size": True,
            "revenue": True,
            "social_presence": True,
        },
        "fraud_risk_scores": {"identity": 0.1, "location": 0.2, "behavior": 0.1},
...
async def test_fraud_risk_impact(pricing_engine, sample_lead_data):
    ...
    high_risk["fraud_risk_scores"] = {"identity": 0.8, "location": 0.7, "behavior": 0.9}
    high_risk_price = await pricing_engine.calculate_price(high_risk)

    # Test low risk lead
    low_risk = sample_lead_data.copy()
    low_risk["fraud_risk_scores"] = {"identity": 0.1, "location": 0.2, "behavior": 0.1}
...
async def test_enrichment_impact(pricing_engine, sample_lead_data):
    ...
    basic = sample_lead_data.copy()
    basic["enrichment_data"] = {"company_size": True}
    basic_price = await pricing_engine.calculate_price(basic)
...
async def test_batch_pricing(pricing_engine):
    ...
    leads = [sample_lead_data(), sample_lead_data(), sample_lead_data()]

    # Calculate prices in batch
    prices = await asyncio.gather(
        *[pricing_engine.calculate_price(lead) for lead in leads]
    )
