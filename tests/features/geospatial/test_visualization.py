...
def visualizer(mock_session, mock_cache):
    return GeospatialVisualizer(session=mock_session, cache=mock_cache, cache_ttl=3600)
...
@pytest.mark.asyncio
async def test_cluster_visualization_generation(
    visualizer, mock_session, mock_cache, sample_point
):
...
@pytest.mark.asyncio
async def test_competitor_proximity_generation(
    visualizer, mock_session, mock_cache, sample_point
):
    ...
    mock_session.execute.side_effect = [
        AsyncMock(return_value=[Mock(id=1, location=sample_point)])(),
